...
- ### Arrays
    ...
    - [-] Implement a vector (mutable array with automatic resizing):
        ...
        - [X] capacity() - number of items it can hold: N/A to python
        - [X] is_empty(): `not arr` 
        - [X] at(index) - returns item at given index, blows up if index out of bounds: `arr[index]`
        - [X] push(item): `append()`
        - [X] insert(index, item) - inserts item at index, shifts that index's value and trailing elements to the right: `insert(index, item)`
        - [X] prepend(item) - can use insert above at index 0: see insert
        - [X] pop() - remove from end, return value: `pop()`
        - [X] delete(index) - delete item at index, shifting all trailing elements left: `pop(index)`
        - [X] remove(item) - looks for value and removes index holding it (even if in multiple places): `remove(item)`
        - [X] find(item) - looks for value and returns first index with that value, -1 if not found: `index(item)`, returns value error if not found 
        - [X] resize(new_capacity) // private function : N/A for python
